...
# Compile asan_options.c: bakes detect_leaks=0 into every binary to prevent
# LeakSanitizer from ptrace-attaching at exit (Mayhem already holds the
# process under ptrace for coverage collection; a second ptrace attach fails,
# causing LSan to call _exit(-1) before edges are recorded → 0-edge "Run Failed").
# Strong symbol wins over the ASan runtime's weak __asan_default_options.
ASAN_OPTS_OBJ="$SRC/mayhem/asan_options.o"
$CC $DEBUG_FLAGS -c "$SRC/mayhem/asan_options.c" -o "$ASAN_OPTS_OBJ"

# 2a) libFuzzer harness (the Mayhem fuzzing binary).
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE $DEFS \
    "$HARNESS" "$ASAN_OPTS_OBJ" $INCS $LINK_LIBS \
    -o /mayhem/fuzz_parse
...
$CC $SANITIZER_FLAGS $DEBUG_FLAGS $DEFS \
    "$STANDALONE_FUZZ_MAIN" "$HARNESS" "$ASAN_OPTS_OBJ" $INCS $LINK_LIBS \
...
for name in fuzz_m2ts_probe fuzz_probe_analyze fuzz_route fuzz_scene; do
    ...
    # asan_options.o bakes detect_leaks=0 to prevent LSan ptrace clash under Mayhem.
    $CC $SANITIZER_FLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE $DEFS \
        "$HARNESS" "$ASAN_OPTS_OBJ" $INCS $FULL_LINK_LIBS \
        -o "/mayhem/$name"
    ...
    $CC $SANITIZER_FLAGS $DEBUG_FLAGS $DEFS \
        "$STANDALONE_FUZZ_MAIN" "$HARNESS" "$ASAN_OPTS_OBJ" $INCS $FULL_LINK_LIBS \
